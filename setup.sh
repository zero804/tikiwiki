...
composer()
{
	...
	PHP_OPTION="--version"
	REQUIRED_PHP_VERSION=72 # minimal version PHP 7.2 but no decimal seperator, no floating point data
	#${PHPCLI} ${PHP_OPTION}
	LOCAL_PHP_VERSION=`"${PHPCLI}" ${PHP_OPTION} | ${GREP} ^PHP | ${CUT} -c5,7`
	#echo ${LOCAL_PHP_VERSION}
	LIKELY_ALTERNATE_PHP_CLI="php73 php7.3 php7.3-cli php72 php7.2 php7.2-cli" # These have been known to exist on some hosting platforms
