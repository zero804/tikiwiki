		...
		<form method="post" action="{$smarty.server.REQUEST_URI|escape}">
			...
			<table class="table">
				...
				<tr>
					...
					<td><a class="timeout" href="{$smarty.server.SCRIPT_NAME}?page=semantic&token={$token.token|escape}">{$token.token|escape}</a></td>
					<td>{$token.label|escape}</td>
					<td><a class="timeout" href="{$smarty.server.SCRIPT_NAME}?page=semantic&token={$token.invert_token|escape}">{$tokens[$token.invert_token].label|escape}</a></td>
				</tr>
				{/foreach}
			</table>
			<p>
