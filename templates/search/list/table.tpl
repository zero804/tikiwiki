...
{if $tableparams.allowtableexpansion eq 'y'}
	{button href='javascript:void(0)' _type="primary" _class="btn btn-primary btn-sm table-expand-toggle" _icon_name="caret-square-o-right" _title="{tr}Expand table{/tr}"}
	{jq}
		$(".table-expand-toggle").click(function(){
			...
			if ( $this.data('expandStatus') != 'expanded' ) {
				...
				$this.find(".icon").setIcon("caret-square-o-left");
			}else{
				...
				$this.find(".icon").setIcon("caret-square-o-right");
			}
		});
	{/jq}
{/if}
...
{if $actions}
	<div class="row">
		...
		<div class="col-sm-4" id="list_input_container"></div>
