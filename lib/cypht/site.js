...
var orig_request = Hm_Ajax_Request();
var new_request = Hm_Ajax_Request();
...
new_request.xhr_fetch = function(config) {
    if(config.data && $('#hm_session_prefix').length > 0) {
        config.data.push({'name': 'hm_session_prefix', 'value': $('#hm_session_prefix').val()});
    }
    orig_request.xhr_fetch(config);
}
Hm_Ajax_Request = function() {
    return new_request;
}
