...
(function($) {
	...
	$('.table-responsive:not(.article-types)').each(function () {
		...
		if (table.find('table:not(.tablesorter) tbody tr').width() - 5 > table.width()) {
			...
			if (screen.width <= 767) {
				$('<div class="toggle-table d-md-none"><button type="button" class="btn btn-link fas fa-toggle-off"></button></div>').insertBefore(table);
				if ($('.toggle-table button').click(function(){
					if ($(this).hasClass('fa-toggle-on')) {
						$(this).removeClass('fa-toggle-on').addClass('fa-toggle-off');
						table.removeClass('large-table').addClass('large-table-no-wrap');
					} else {
						$(this).removeClass('fa-toggle-off').addClass('fa-toggle-on');
						table.removeClass('large-table-no-wrap').addClass('large-table');
					}
				}));
				...
				table.find('table th').each(function (e) {
					...
					if ($(this).find('div').hasClass('form-check')) {
						if (e == 0) $(this).addClass('selectall');
						header = "";
					}
