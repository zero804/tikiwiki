	...
	$(document).on('click', 'a.click-modal, .click-modal a', $.clickModal({
		...
		success: function (data) {
			window.location.href =
				$(this).data('modal-submit-redirect-url') ?? window.location.href.replace(/#.*$/, '');
		}
	}));
...
(function($) {
	...
	$('.table-responsive:not(.article-types)').each(function () {
		...
		if (table.find('table:not(.tablesorter):not(.caltable) tbody tr').width() - 10 > table.width()) {
			...
			if (screen.width <= 767) {
				...
				var checkall = false;
				table.find('table.table th, table.caltable tbody tr:first').each(function (e) {
					...
					table.find('table.table tbody tr, table.caltable tbody tr:not(:first)').each(function () {
