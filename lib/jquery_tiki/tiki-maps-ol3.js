(function () {
	...
	function getBaseLayers(tilesets)
	{
...
/*							container.map.setLayerZIndex(vectors, vectorLayerList.length * 1000);
			...
			setTimeout(function () {
				...
				var tilesets = $(container).data("tilesets");
				if (tilesets) {
					tilesets.replace(/\s*/g, "").split(",");
					if (typeof tilesets === "string") {
						tilesets = [tilesets];
					}
				}
				var layers = getBaseLayers(tilesets);
...
*/
				...
				if ($(container).data("tooltips")) {
					...
					var displayFeatureInfo = function (pixel) {
						...
						$mapBootstrapDummy.tooltip("destroy");		// NB dispose in bootstrap4
						if (! feature) {
							return;
						}
				...
				if ($(container).data("popup-style")) {
					...
					selectionInteraction.on('select', function (e) {
						...
						$mapBootstrapDummy.tooltip("destroy").css({		// NB dispose in bootstrap4
							left: pixel[0] + "px",
							top: pixel[1] + "px"
						});
						...
						switch ($(container).data('popup-style')) {
							...
							case 'popup':
							default:
								$mapBootstrapDummy.popover("destroy").popover({		// NB dispose in bootstrap4
									trigger: "manual",
									html: true,
									content: tr("Loading...")
									...
									if (type && object) {
										$(container).loadInfoboxPopup({
											...
											element: this,
											callback: function (event, $html) {
												var $popover = $(".popover.in:first");	// NB .show in bootstrap4
												$html.find("a.service-dialog").remove();
												var title = $html.find("h1").remove().text() || feature.get("content");
												$(".popover-title", $popover).text(title).show();	// NB .popover-header in bootstrap4
												$(".popover-content", $popover).empty().append($html);	// NB .popover-body in bootstrap4
												$popover.mouseleave(function () {
													$popover.popover("destroy");		// NB dispose in bootstrap4
													selectionInteraction.getFeatures().clear();
												});
											}
										});
